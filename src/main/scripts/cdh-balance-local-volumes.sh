...
sudo -u hdfs HADOOP_USER_CLASSPATH_FIRST=true HADOOP_CLASSPATH=/tmp/volume-balancer-${DATE} hadoop jar ${ROOT_FOLDER}/lib/volume-balancer-${project.version}.jar org.apache.hadoop.hdfs.server.datanode.VolumeBalancer $@ | tee -a ${ROOT_FOLDER}/logs/log-$(date +%s).log
